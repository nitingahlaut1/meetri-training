...
for (var i = 0; i < arr.length; i++) {
    if (!obj.hasOwnProperty(arr[i].category)) {
        ...
        for (var j = 0; j < arr.length; j++) {
            if (arr[j].category === arr[i].category && !temp.hasOwnProperty(arr[j].type)) {
                temp[arr[j].type] = [];
                temp[arr[j].type].push("".concat(arr[j].brand, " ").concat(arr[j].model));
            }
            else if (arr[j].category === arr[i].category && !temp[arr[j].type].includes("".concat(arr[j].brand, " ").concat(arr[j].model))) {
                temp[arr[j].type].push("".concat(arr[j].brand, " ").concat(arr[j].model));
            }
        }
        obj[arr[i].category] = temp;
    }
}
console.log(obj);
